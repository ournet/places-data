...
export class DataPlaceHelper {
    ...
    static createKeyInAdmin1(place: Partial<Place>) {
        if (place.countryCode && place.admin1Code && place.featureCode && place.featureCode.trim().toUpperCase() !== 'ADM1') {
            return DataPlaceHelper.formatKetInAdmin1(place.countryCode, place.admin1Code);
        }
    }
    ...
    static createKeyAdmin1(place: Partial<Place>) {
        if (place.countryCode && place.featureClass && place.featureCode && place.featureClass === 'A' && place.featureCode.trim().toUpperCase() === 'ADM1') {
